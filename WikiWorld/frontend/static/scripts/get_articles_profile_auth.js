        ...
        success: function (response) {
            for (var i = 0; i < response.length; i++) {
                if (response[i].user.toString() === id) {
                    ...
                    var but2 = '<form id="form_edit_article">' + `<button class="editbutton-profile"><a href="/edit_article/${response[i].id}/"><img class="editbutton-profile-svg" src="/static/img/pencil.svg"></a></button>` + '</form>';
